...
LatencyTuple launchConstL1LatKernelBenchmark(int N, int* error) {
    ...
    do {
        ...
        if (h_time == nullptr) {
            printf("[CONSTL1_LAT.CPP]: malloc h_time Error\n");
            *error = 1;
            break;
        }
        ...
        if (error_id != hipSuccess) {
            printf("[CONSTL1_LAT.CPP]: hipMalloc d_time Error: %s\n", hipGetErrorString(error_id));
            *error = 2;
            break;
        }
        ...
        dim3 Db = dim3(1);
        dim3 Dg = dim3(1, 1, 1);
        hipLaunchKernelGGL(constL1_lat, Dg, Db, 0, 0, N, d_time);

        error_id = hipDeviceSynchronize();
        error_id = hipGetLastError();
        if (error_id != hipSuccess) {
            printf("[CONSTL1_LAT.CPP]: Kernel launch/execution with clock Error: %s\n", hipGetErrorString(error_id));
            *error = 5;
            break;
        }
        error_id = hipDeviceSynchronize();
        ...
        if (error_id != hipSuccess) {
            printf("[CONSTL1_LAT.CPP]: hipMemcpy d_time Error: %s\n", hipGetErrorString(error_id));
            *error = 6;
            break;
        }
        error_id = hipDeviceSynchronize();
        ...
        hipLaunchKernelGGL(constL1_lat_globaltimer, Dg, Db, 0, 0, N, d_time);

        error_id = hipDeviceSynchronize();
        error_id = hipGetLastError();
        if (error_id != hipSuccess) {
            printf("[CONSTL1_LAT.CPP]: Kernel launch/execution with globaltimer Error: %s\n", hipGetErrorString(error_id));
            *error = 5;
            break;
        }
        error_id = hipDeviceSynchronize();
