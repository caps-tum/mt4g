...
namespace benchmark {
    namespace amd {
        double measureL3ReadBandwidth(size_t l2SizeBytes, size_t l3SizeBytes)
        {
            ...
            const size_t arraySizeBytes = util::max(l2SizeBytes * (util::getNumXCDs().value_or(1) + 2), l3SizeBytes / 4);
        ...
        CacheBandwidthResult measureL3ReadBandwidthSweep(size_t l2SizeBytes, size_t l3SizeBytes) 
        {
            ...
            size_t arraySizeBytes = util::max(l2SizeBytes * (util::getNumXCDs().value_or(1) + 2), l3SizeBytes / 4);
