...
__global__ void scalarL1LineSizeKernel(uint32_t *timingResults, size_t steps, uint32_t stride) {
    ...
    for (uint32_t k = 0; k < measureLength; ++k) {
        ...
        uint64_t start, end;
        // Both stride and index may be VGPR (warm-up loop uses vector arithmetic).
        // Perform all v_readfirstlane_b32 calls INSIDE the asm block where the compiler
        // allocates "=&s" outputs in SGPR — avoids the allocator moving the result back
        // to VGPR when stored in a C++ variable.
