...
static std::tuple<uint64_t, double, double> sharedWriteBandwidthStaticLauncher(uint32_t numThreads, size_t reps) 
{
    ...
    sharedWriteBandwidthStaticKernel<<<1, numThreads>>>(d_timings, elementsPerThread, reps);
    ...
    CacheBandwidthResult measureSharedWriteBandwidthStaticSweep() 
    {
        ...
        for (uint32_t numThreads = minNumThreads; numThreads <= maxNumThreads; numThreads *= 2)
        {
            ...
            for (size_t reps = minReps; reps <= maxReps; reps *= 2)
            {
                ...
                auto [cycles, timeS, bandwidth] = sharedWriteBandwidthStaticLauncher(numThreads, reps);
