...
std::tuple<std::vector<uint32_t>, std::vector<uint32_t>> textureAmountLauncher(size_t textureSizeBytes, size_t textureFetchGranularityBytes, uint32_t baseCore, uint32_t testCore) {
    ...
    textureAmountKernel<<<1, util::getNumberOfCoresPerSM()>>>(texBase, texTest, d_timingResultsBaseCore, d_timingResultsTestCore, textureSizeBytes / textureFetchGranularityBytes, baseCore, testCore);
