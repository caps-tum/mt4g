...
static constexpr auto SAMPLE_SIZE = 48;// Tries


__global__ void constantL15FetchGranularityKernel(uint32_t *timingResults, size_t stride) {
...
std::vector<uint32_t> constantL15FetchGranularityLauncher(size_t fetchGranularityToTestBytes) { 
    ...
    constantL15FetchGranularityKernel<<<1, 1>>>(d_timingResults, fetchGranularityToTestBytes / sizeof(uint32_t));
...
namespace benchmark {
    namespace nvidia {
        CacheSizeResult measureConstantL15FetchGranularity(size_t constantL1FetchGranularityBytes) {
            ...
            for (size_t currentFetchGranularityBytes = constantL1FetchGranularityBytes; currentFetchGranularityBytes <= MAX_EXPECTED_LINE_SIZE; currentFetchGranularityBytes += sizeof(uint32_t)) {
                timings[currentFetchGranularityBytes] = constantL15FetchGranularityLauncher(currentFetchGranularityBytes);
            }
