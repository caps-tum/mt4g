...
def plot_block_peak(grid: BlockSweepGrid, title: str, outdir: Path, outbase: str, dpi: int) -> None:
    """Peak-bandwidth-per-block figure for (possibly truncated) block sweeps.

    The bandwidth search stops descending in thread count once a block's
    bandwidth drops, so each block is measured at a *different* set of thread
    counts and the classic "one line per block, x=threads" curves are truncated
    at different points -- no longer comparable.  Instead this figure shows, for
    every block size:

      * the peak (max) measured bandwidth -- the primary result, block sizes
        connected by a line to reveal the trend;
      * every individual measurement that was actually performed, as scatter
        points;
      * the launch thread count of each measurement, annotated next to its point
        (bold/red on the peak point, muted on the others).

    Block sizes sit on evenly spaced categorical x-positions: their raw values
    are geometric (e.g. 132, 264, 528, ...) and would crowd a linear axis.
    """
    ...
    for x, bi in enumerate(range(len(blocks))):
        ...
        for t, v in measured:
            ...
            if v == peak:
                # The peak is always the block's topmost point: lift its label
                # above the marker so it clears the diamond, the trend line and
                # the other (lower) measurement labels for this block.
                ax.annotate(
                    str(t), (x, v),
    ...
    # Headline: peak bandwidth per block, connected to show the trend.
    ax.plot(
        xpos, peak_vals,
    ...
    # The number beside every point is its launch thread count (block dim); make
    # that explicit via the legend title so the annotations are unambiguous.
