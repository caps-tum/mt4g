...
namespace util {
    std::vector<uint32_t> getLogicalToPhysicalCUsLUT() {
        ...
        for (uint32_t xcdId = 0; xcdId < util::getNumXCDs().value_or(1); ++xcdId) {
            uint32_t numCUs = 304;
            ...
            for (uint32_t cuIdOnXCD = 0; cuIdOnXCD < numCUs; ++cuIdOnXCD) {
                ...
                translationKernel<<<util::getNumXCDs().value_or(1) * 2, 1, 0, stream>>>(d_resultBuffer, cuIdOnXCD, cuIdOnXCD / 8);
